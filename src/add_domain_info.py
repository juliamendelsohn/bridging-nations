...
for country_code in ['DE','PL','TR','ES','PT']:
    user_hashtag_file = f'/shared/2/projects/cross-lingual-exchange/data/domains/domains_by_country/{country_code}_users.json'
...
def add_all_hashtag_info(old_df,country_hashtags,user_hashtags,c1,c2):
    ...
    for uid in user_ids:
        if uid in user_hashtags:
            ...
            info[f'{c1}_num_domains'] = len(c1_hashtags)
            info[f'{c2}_num_domains'] = len(c2_hashtags)

            info[f'{c1}_frac_domains'] = len(c1_hashtags) / len(user_hashtags[uid])
            info[f'{c2}_frac_domains'] = len(c2_hashtags) / len(user_hashtags[uid])

            info[f'{c1}_jacc_domains'] = len(c1_hashtags) / len(set(user_hashtags[uid]).union(set(country_hashtags[c1])))
            info[f'{c2}_jacc_domains'] = len(c2_hashtags) / len(set(user_hashtags[uid]).union(set(country_hashtags[c2])))
...
def main():
    logodds_path = '/shared/2/projects/cross-lingual-exchange/data/domains/logodds_country_domains/'
    ...
    for i in range(len(country_pairs)):
        ...
        logodds_file = os.path.join(logodds_path,f'domain_log_odds_{a1}_{a2}_scaled.tsv')
        ...
        new_df.to_csv(os.path.join(datasheets_dir,f'{l1}_{l2}_with_domain_and_neighbor_info.tsv'),sep='\t',index=False)
